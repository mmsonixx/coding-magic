...
<div class="divider"></div>
  ...
  `,e.querySelectorAll("[data-scientist-task]").forEach(t=>{t.addEventListener("click",t=>{let u=F[Number(t.currentTarget.dataset.scientistTask)],s=u.action();e.querySelector("[data-scientist-answer]").innerHTML=`
        <h3 class="guess-scientist__answer-title">${u.title}</h3>
        ${Array.isArray(s)?`<ul class="guess-scientist__answer-list">
      ${s.map(e=>`<li>${"string"==typeof e?e:$(e)}</li>`).join("")}
    </ul>`:"object"==typeof s?`<p class="guess-scientist__answer-text">${$(s)}</p>`:`<p class="guess-scientist__answer-text">${s}</p>`}
      `})}))}}],H=document.querySelector("[data-game-container]"),T=document.querySelectorAll(".dropdown__link");function N(e,t){if(!t)return void console.error("Контейнер не знайдено");if(0===e.length){t.innerHTML='<p class="no-games">Ігор у цій категорії поки немає</p>';return}t.innerHTML=e.reduce((e,t)=>e+`
    ...
    `,""),e.forEach(e=>{e.init&&e.init()})}N(B,H),T.forEach(e=>{e.addEventListener("click",e=>{e.preventDefault();let t=e.target.dataset.category;N("all"===t?B:B.filter(e=>e.category===t),H)})});
//# sourceMappingURL=coding-magic.8838a00f.js.map
